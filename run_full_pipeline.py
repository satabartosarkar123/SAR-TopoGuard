...
from pure_python_adam import PurePythonAdam
from pure_python_adam import PurePythonAdam
...
def build_loaders(split="train", max_images=None, batch_size=4, shuffle=True):
    ...
    loader = DataLoader(ds, batch_size=batch_size, shuffle=shuffle, num_workers=0, pin_memory=False)
...
def phase_2_smoke_test():
    ...
    optG = PurePythonAdam(netG.parameters(), lr=1e-4, betas=(0.5, 0.999))
    optD = PurePythonAdam(netD.parameters(), lr=1e-4, betas=(0.5, 0.999))
