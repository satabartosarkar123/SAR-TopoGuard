...
def phase_2_smoke_test():
    ...
    print('Starting netG init'); netG = UNetGenerator(in_channels=2, out_channels=3).cuda(); print('NetG init done')
    print('Starting netD init'); netD = PatchGANDiscriminator(in_channels=5).cuda(); print('NetD init done')
    print('Applying init_weights to G'); netG.apply(init_weights)
    print('Applying init_weights to D'); netD.apply(init_weights)

    print('Init optG'); optG = torch.optim.Adam(netG.parameters(), lr=1e-4, betas=(0.5, 0.999))
